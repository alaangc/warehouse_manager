...
export async function createPerformanceFixture(
  options: {
    ...
    allowLocalPostgres?: boolean;
  } = {},
) {
  const salesIntervalSeconds = options.salesIntervalSeconds ?? 1;
  if (!Number.isSafeInteger(salesIntervalSeconds) || salesIntervalSeconds < 1)
    throw new Error('Sales interval must be a positive integer');
  if (process.env.TEST_POSTGRES_ADMIN_URL && !options.allowLocalPostgres)
    throw new Error(
  ...
  try {
    ...
    await database.transaction().execute(async (transaction) => {
      await sql
        .raw(seed.replaceAll("interval '1 second'", `interval '${salesIntervalSeconds} seconds'`))
        .execute(transaction);
    });
